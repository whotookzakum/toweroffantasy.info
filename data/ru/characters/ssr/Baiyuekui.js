const baiyuekui = {
    ...
    weapon: {
        ...
        bonusEffect: {
            effect1: {
                title: "Боевой дух",
                description: "При входе в бой финальный урон увеличивается на **15%**, но персонаж получает эффект Распада клеток, теряя **1%** от текущего здоровья каждые 5 секунд. Распад клеток останавливается, когда здоровье падает ниже **50%**, при этом сохраняя увеличение урона от Боевого духа. Смена оружия на другое отменяет эффект Боевого духа."
            }
        },
        advancement: {
            ...
            star3: `Пробивание увеличивается на **15%**. При пробивании щита цели на её месте создаётся Подавляющая бомба, мгновенно наносящая урон, равный **680%** от АТК всем целям поблизости. Если в радиусе взрыва только один враг, наносит урон, равный **1500%** от АТК.`,
    ...
    matrix: {
        ...
        set4: `За каждые 0,6 секунд выполнения серии комбо-ударов **следующая** серия нанесёт **первому** поражённому врагу в радиусе 5 метров дополнительный урон, равный **106%/122%/140%/160%** от АТК, вплоть до максимума в **530%/610%/700%/800%** от АТК. После этого эффективность пробивания увеличивается на **7%/14%/21%/28%** в течение 4 секунд (эффект не складывается). Эффект набора действует в фоновом режиме, и при наличии нескольких таких наборов действовать будет лишь набор с самым большим числом звёзд.`
    },
    
    awakening: {
        trait1200: `Когда Бай Юэкуй вступает в бой, АТК Напряжением увеличивается на **21%**. Эффект увеличения ослабевает со временем и длится 120 секунд. Эффект сбрасывается спустя **5** секунд после выхода из боя. Наносимый по Колючему угрю и Земляному рою урон увеличивается на **50%**.`,

        trait4000: `Когда Бай Юэкуй вступает в бой, АТК Напряжением увеличивается на **33%**. Эффект увеличения ослабевает со временем и длится 120 секунд. Эффект сбрасывается спустя **5** секунд после выхода из боя. Наносимый по Колючему угрю и Земляному рою урон увеличивается на **50%**.`,
    },
