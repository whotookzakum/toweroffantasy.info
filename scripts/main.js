...
function toggleModal({
    ...
    function handleOffClick(event) {
        ...
        if ($(event.target).hasClass("btn-close") && !parents.includes(modal[0])) {
            return;
        }

        if (
            (parents.includes($(".modal-content")[0]) ||
                parents.includes($(".navbar")[0]) ||
                parents.includes($(".popover")[0])) &&
            !$(event.target).hasClass("btn-close")
        ) {
            return;
        }

        hide();
        $("body").off("click", handleOffClick);
    ...
    function show() {
        const displayContents = () => {
            ...
            modal.one("transitionend", () => {
                root.setAttribute("data-scrollTop", root.scrollTop);
                $("#modal-menu").one("transitionend", () => {
                    root.setAttribute("style", "overflow: hidden; padding-right: 0;");
                    document.body.setAttribute("style", "overflow: hidden; padding-right: 0;");
                });
                $("body").on("click", handleOffClick);
                $(window).keyup(handleEsc);
            });
        };

        $("#page-bg-img").css("opacity", 0);
        $("#modal-bg-img").one("load", handleImgLoad);
        $("#modal-menu").css("opacity", "0");

        modal.trigger("tinyCoolEpicNotScuffedModalOpen");
        displayContents();
    }
    ...
    function hide() {
        ...
        $("#modal-bg-img").off("load", handleImgLoad);
        $("#modal-menu").css("display", "block");
        $("#modal-menu").focus();
        $("#modal-menu").removeAttr("style");

        modal.trigger("tinyCoolEpicNotScuffedModalClose");
        modal.attr("aria-hidden", "true");
        modal.removeClass("show");
        modal.one("transitionend", () => {
            modal[0].scrollTop = 0;
            modal.removeAttr("style");
            modal.trigger("tinyCoolEpicNotScuffedModalCloseButForReal");
        });
        ...
        root.scrollTop = root.getAttribute("data-scrollTop");
    }
}
...
function setModalData(i) {
    ...
    if (document.URL.includes("simulacra")) {
        ...
        $("#wep-effect").html(parseHTML(md.renderInline(i.wepEffect)));
            ...
            for (let currentChip = 0; currentChip < i.recChips.length; currentChip++) {
                ...
                $("#recommended-chips-wrapper").append(
                    `<div class="chip-recommended col-2">
												<img class="chip-img" src="images/chip/${i.recChips[currentChip][0]}.png" width="128" height="128">
